...
Expected<Config> parse_cli(int argc, char **argv) {
  ...
  if (config.json_path.empty()) {
    return std::unexpected(
        Error{ErrorCode::InvalidArguments, "Missing --json parameter"});
  }
  if (config.schema_path.empty()) {
    return std::unexpected(
        Error{ErrorCode::InvalidArguments, "Missing --schema parameter"});
  }

  // TODOS: replace with dot-env
  ...
  if (config.send_email && !config.api_email) {
    return std::unexpected(
        Error{ErrorCode::EnvironmentMissing,
