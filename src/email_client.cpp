...
Expected<void> EmailClient::send_status(bool success, std::string_view message) {
    if (!config_.api_email) return {};

    CURL* curl = curl_easy_init();
    if (!curl) return std::unexpected(Error{ErrorCode::InternalError, "Failed to init CURL for email"});
    ...
    if (res != CURLE_OK || (http_code != 200 && http_code != 202)) {
        return std::unexpected(Error{ErrorCode::EmailFailed, 
